...
#define gpuErrorRep(ans, A, Anew, A_d, max) { gpuAssert((ans), __FILE__, __LINE__, A, Anew, A_d, max); }
...
int main(int argc, char* argv[])
{
    ...
    gpuErrorRep( cudaMalloc(&A_d, sizeof(double*) * netSize * netSize), A_h, Anew, A_d, max);
    gpuErrorRep( cudaMalloc(&Anew, sizeof(double*) * netSize * netSize), A_h, Anew, A_d, max );
    gpuErrorRep( cudaMalloc(&max, sizeof(double*) * netSize * netSize), A_h, Anew, A_d, max );
    gpuErrorRep(cudaMalloc(&d_error, sizeof(double)), A_h, Anew, A_d, max);

    //setting A_d matrix to zero's
    gpuErrorRep( cudaMemset(A_d, 0, sizeof(double) * netSize), A_h, Anew, A_d, max );
    ...
    gpuErrorRep( cudaMalloc(&d_tempStorage, d_tempStorageBytes), A_h, Anew, A_d, max);
    ...
    gpuErrorRep( cudaGetLastError(), A_h, Anew, A_d, max );
    ...
    while (error > minError && iteration < maxIterations)
    {
        ...
        if(!graphCreated) {
            ...
            gpuErrorRep( cudaStreamCreate(&stream), A_h, Anew, A_d, max );
            gpuErrorRep( cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal), A_h, Anew, A_d, max );
            ...
            gpuErrorRep( cudaStreamEndCapture(stream, &graph), A_h, Anew, A_d, max );
            gpuErrorRep( cudaGraphInstantiate(&graphInstance, graph, NULL, NULL, 0), A_h, Anew, A_d, max );
            printf("graph instantiated!\n");
            graphCreated = true;
        }

        gpuErrorRep( cudaGraphLaunch(graphInstance, stream), A_h, Anew, A_d, max );
        gpuErrorRep( cudaStreamSynchronize(stream), A_h, Anew, A_d, max );
        ...
        //copy error to CPU
        gpuErrorRep( cudaMemcpy(&error, d_error, sizeof(double), cudaMemcpyDeviceToHost), A_h, Anew, A_d, max );
    ...
    if (toPrintResult) {
        //print matrix
        gpuErrorRep(cudaMemcpy(A_h, Anew, sizeof(double) * netSize * netSize, cudaMemcpyDeviceToHost), A_h, Anew, A_d, max);
        gpuErrorRep(cudaDeviceSynchronize(), A_h, Anew, A_d, max)
